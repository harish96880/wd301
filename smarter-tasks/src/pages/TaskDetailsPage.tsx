...
const TaskDetailsPage: React.FC = () => {
  ...
  return (
    <div className="taskDetail bg-white shadow-md rounded-md p-4 m-8 fw-bold">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-medium">{task?.title}</h3>
      </div>
      <p className="text-white-600">{task?.description}</p>
      <p className="text-white-600">{task?.dueDate}</p>
    </div>
  );
};
